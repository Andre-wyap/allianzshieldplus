// Pure-JS MD5 — Cloudflare Edge compatible (Web Crypto API does not support MD5)
// RFC 1321 implementation used for Senang Pay signature generation and verification.
...
export function md5Hex(input: string): string {
  ...
  // Allocate padded message in 32-bit words (little-endian)
  const paddedLen = (((len8 + 8) >>> 6) + 1) << 4;
  const M = new Array<number>(paddedLen).fill(0);
  for (let i = 0; i < len8; i++) M[i >>> 2] |= bytes[i] << ((i & 3) << 3);
  M[len8 >>> 2] |= 0x80 << ((len8 & 3) << 3);
  M[paddedLen - 2] = (len8 << 3) >>> 0;
  M[paddedLen - 1] = (len8 >>> 29) >>> 0;
